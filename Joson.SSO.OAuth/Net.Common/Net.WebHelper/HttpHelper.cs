...
namespace Net.Common
{
    ...
    #region HttpHelper类使用方法

    public class JosonHttpHelper
    {
        ...
        #region  最简单的Post与Get的写法

        public virtual void GetHtml(string strURL, bool IsPost = false)
        {

            HttpHelper http = new HttpHelper();
            HttpItem item = new HttpItem()
            {
                URL = strURL,                       //"http://www.sufeinet.com",//URL这里都是测试     必需项
                Method = IsPost ? "post" : "get",   // URL 可选项 默认为Get
            };
            //得到HTML代码
            HttpResult result = http.GetHtml(item);
            item = new HttpItem()
        ...
        #region HttpHelper设置Header参考的方法

        public void SetHeader(string strURL, bool IsPost = false)
        {

            HttpHelper http = new HttpHelper();
            HttpItem item = new HttpItem()
            {
                URL =strURL,                    // "http://www.sufeinet.com",//URL     必需项
                Encoding = null,                //编码格式（utf-8,gb2312,gbk）     可选项 默认类会自动识别
                //Encoding = Encoding.Default,
                Method = IsPost ? "post" : "get",   // URL 可选项 默认为Get
            };
        ...
        #region   HttpHelper获取图片的方式

        public void RmoteGetImage(string strURL, bool IsPost = false)
        {

            HttpHelper http = new HttpHelper();
            HttpItem item = new HttpItem()
            {
                URL =strURL,                // "http://www.sufeinet.com",//URL必需项
                Encoding = null,            //编码格式（utf-8,gb2312,gbk）可选项 默认类会自动识别
                //Encoding = Encoding.Default,
