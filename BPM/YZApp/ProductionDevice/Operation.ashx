<%@ WebHandler Language="C#" Class="YZSoft_ProductionDeviceOperator" %>
...
public class YZSoft_ProductionDeviceOperator : IHttpHandler
{
            ...
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["BPMDB"].ConnectionString))
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        cmd.Connection = cn;
                        cmd.CommandText = String.Format("DELETE FROM iDemoDevice WHERE id={0}", id);
