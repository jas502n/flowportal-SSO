...
public class YZExcelService
{
    ...
    protected void LoadDataSet(HSSFWorkbook book, Dictionary<string, string> reportParams, ref DataSet dataset)
    {
        ...
        for (int r = 1; r <= sheetDS.LastRowNum; r++)
        {
            ...
            dataset.Tables.Add(QueryManager.CurrentProvider.Query2Table(datasourceName,query,resultTableName));
        }
    }
        ...
        foreach (GridDefine grid in gridDefines)
        {
            ...
            if (dataTable != null)
            {
                ...
                foreach (Group group in grid.Groups)
                {
                    ...
                    if (fill == null || gridTagRowCount == 0)
                        continue;

                    int startRowIndex = grid.StartRow;
                    string curvalue = sheetfill.GetRow(startRowIndex).GetCell(fill.ColumnIndex).StringCellValue;
                    for (int i = 0; i < gridTagRowCount; i++)
                    {
                        string value = null;
                        int rowIndex = grid.StartRow + i;
                        if (i != gridTagRowCount-1)
                        {
                            value = sheetfill.GetRow(rowIndex).GetCell(fill.ColumnIndex).StringCellValue;
