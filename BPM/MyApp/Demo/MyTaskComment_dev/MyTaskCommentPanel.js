...
importJS("MyApp/Demo/MyTaskComment_dev/MyTaskCommentGrid.js");
importJS("MyApp/Demo/MyTaskComment_dev/MyTaskCommentStore.js");
...
YZSoft.BPM.MyTaskCommentPanel = Ext.extend(Ext.Panel, {
    constructor: function (config) {
        ...
        this.store = new YZSoft.BPM.Store.MyTaskCommentStore({
            baseParams: { limit: YZSoft.EnvSetting.PageSize.BPM.historyAllAccessable, HistoryTaskType: 'AllAccessable', SpecProcessName: config.SpecProcessName, byYear: config.byYear === false ? 0 : 1 },
            listeners: {
                scope: this,
                beforeload: function () {
        ...
        this.grid = new YZSoft.BPM.Grid.MyTaskCommentGrid({
            store: this.store,
            sortable: false,
            listeners: {
                scope: this,
        ...
        var cfg = {
            ...
            layout: 'border',
            items: [this.grid],
            tbar: [this.btnPublic, '|', this.btnMore, '|', this.btnTrace, '->', this.btnClear, ' ', this.cmbProcessName, ' ', this.searchField],
            //tbar: [this.btnPublic, '|', this.btnMore, '|', this.btnTrace, '->', this.btnClear, ' ', this.periodPicker, ' ', this.cmbProcessName, ' ', this.searchField],
            listeners: {
                scope: this,
                activate: function () {
                    if (!this.loaded) {
        ...
        YZSoft.BPM.MyTaskCommentPanel.superclass.constructor.call(this, cfg);
    ...
    initComponent: function () {
        YZSoft.BPM.MyTaskCommentPanel.superclass.initComponent.call(this);
        this.updateStatus();
    },

    OpenForm: function (item) {
        YZSoft.BPM.Grid.MyTaskCommentGrid_OpenTaskForRead(this.grid.id, item.TaskID, item.SerialNum);
    },
    ...
    updateStatus: function () {
        ...
        if (recs.length == 0) {
            ...
            Ext.each(recs, function (rec) {
                if (!rec.data.Perm.PickBackRestart)
                    pickbackRestart = false;
