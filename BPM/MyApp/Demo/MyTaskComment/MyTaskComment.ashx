    ...
    public class MyTaskComment : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            using (BPMConnection cn = new BPMConnection())
            {
                ...
                string query="WITH A AS(SELECT TaskID FROM BPMSecurityTACL WITH(INDEX(YZIX_BPMSecurityTACL_SID)) WHERE SID IN({0}) AND AllowRead=1 AND ExtDeleted=0),";
                query=String.Format(query,sidstr);
                StringBuilder _sb=new StringBuilder();
                _sb.Append(query);
                _sb.Append("B AS(SELECT TaskID FROM BPMInstProcSteps WHERE (OwnerAccount=@UserAccount OR AgentAccount=@UserAccount OR ConsignOwnerAccount=@UserAccount) AND ExtDeleted=0),");
                _sb.Append("C AS(SELECT TaskID TID FROM A UNION SELECT TaskID TID FROM B),");
                ...
                _sb.Append("F AS(SELECT BPMInstTasks.*,E.AppName+':'+E.Comment as Comment,E.LastUpdateDate FROM C INNER JOIN BPMInstTasks ON C.TID=BPMInstTasks.TaskID INNER JOIN E ON BPMInstTasks.TaskID=E.TaskId");
