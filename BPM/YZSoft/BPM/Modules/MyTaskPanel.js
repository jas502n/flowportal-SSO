...
YZSoft.BPM.MyTaskPanel_OpenTaskForProcess = function(gridid, taskid, stepid, sn) {
...
YZSoft.BPM.MyTaskPanel = Ext.extend(Ext.Panel, {
    constructor: function (config) {
        ...
        this.store = new YZSoft.Data.JsonStore({
            ...
            proxy: new Ext.data.HttpProxy({
                method: 'GET',
                url: 'YZSoft/BPM/StoreDataService/MyTask.ashx'
            }),
    ...
    RenderSN: function (value, p, record) {
        return String.format("<a href='#' onclick=\"YZSoft.BPM.MyTaskPanel_OpenTaskForProcess('{0}',{1},{2},'{3}')\">{4}</a>",
            this.grid.id,
            record.data.TaskID,
            record.data.StepID,
    ...
    OpenForm: function (item) {
        YZSoft.BPM.MyTaskPanel_OpenTaskForProcess(this.grid.id, item.TaskID, item.StepID, item.SerialNum);
    },
