...
YZSoft.BPM.FormTemplate_Open = function(gridid, pn, draftid, owner) {
...
YZSoft.BPM.FormTemplate = Ext.extend(Ext.Panel, {
    constructor: function (config) {
        ...
        this.store = new YZSoft.Data.JsonStore({
            ...
            proxy: new Ext.data.HttpProxy({
                method: 'GET',
                url: 'YZSoft/BPM/StoreDataService/FormTemplateData.ashx'
            }),
            listeners: {
                scope: this,
                load: function () {
        ...
        this.btnDelete = new Ext.Button({
            ...
            scope: this,
            handler: function () {
                YZSoft.BPM.TaskOption.DeleteFormTemplates(this.grid);
            }
        });
        ...
        YZSoft.BPM.FormTemplate.superclass.constructor.call(this, cfg);
    ...
    initComponent: function () {
        YZSoft.BPM.FormTemplate.superclass.initComponent.call(this);
        this.updateStatus();
    },

    RenderProcessName: function (value, p, record) {
        return String.format("<div><a href='#' onclick=\"YZSoft.BPM.FormTemplate_Open('{0}','{1}','{2}','{3}')\">{4}</a></div>",
            this.grid.id,
    ...
    OpenForm: function (item) {
        YZSoft.BPM.FormTemplate_Open(this.grid.id, item.ProcessName, item.DraftID, item.Owner);
    },
