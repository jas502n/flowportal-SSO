...
public class YZSoft_MyApp_Task_MyRFC : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
		...
		// Connection String of web.config
                ...
                using (SqlConnection cn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["BPMDB"].ConnectionString))
                {
                    ...
                    string email = "";
                    bc.Open("192.168.0.245", loginaccount, pwd);
                    ...
                    //{
                    //获取邮件地址
                    string sql = "select distinct  case when HandlerAccount IS null then OwnerAccount collate Chinese_PRC_CI_AS else HandlerAccount end as OwnerAccount from BPMInstProcSteps where TaskID=" + taskid;
                    ...
                    for (int j = 0; j < dtUserInfo.Rows.Count; j++)
                    {
                        ...
                        //SendNotify(YZAuthHelper.LoginUserAccount, Title, msgStr.ToString());
                    }
    ...
    public void SendNotify(string recevier, string title, string msg)
    {
        ...
        RootObj.SendNotify(recevier, title, 14400000, msg);
    }
}
